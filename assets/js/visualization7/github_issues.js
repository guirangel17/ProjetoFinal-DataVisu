  ...
  d3.select("svg").remove();
	...
	d3.csv(arquivo, function(error, data) {
		...
		repositories.sort(function(a, b) {
			if (a.num_issues > b.num_issues) return 1;
			if (a.num_issues < b.num_issues) return -1;
			else return 0;
		}).reverse();

		data = repositories.slice(0,20); // Top 20 linguagens com mais issues
		...
		var yhist = d3.scale.linear()
							.domain([0, d3.max(data, function(d) { return d.num_issues; })])
							.range([h, 0]);
		...
		var bar = svg.selectAll(".bar")
					...
					.on("mouseover", function(d) {
							...
							div.html("<b>" + d.language
									+ "</b><br/>"
									+ d.num_issues.toLocaleString('pt-BR') + " issues <br/>")
		...
		bar.append("rect")
			...
			.attr("y", function(d) { return yhist(d.num_issues); })
			.attr("height", function(d) { return h - yhist(d.num_issues); });
